...
Histogram.prototype.topColors = function() {
  ...
  var binCutoff = MIN_BINS;
  var colors = [];

  for (var i = 0; ( i < bins.length && i < binCutoff); i++) {
...
Histogram.prototype.displayColors = function(colors) {
    ...
    if(colors.length < 16) {
        ...
        textElement.classList.add('palette-error');
        textElement.innerHTML += "You need at least 16 colors to generate a syntax theme."
        paletteDisplay.append(textElement);
        return;
    }

    outputAtomSyntaxColorFile(colors);

    addRandomColorToDropDown();
    var randomizeColorButton = document.getElementById('random-button');
    var fitColorButton = document.getElementById('fit-button');
    var fileButton = document.getElementById('file-button');
    randomizeColorButton.style.display = 'inherit';
    fitColorButton.style.display = 'inherit';
    fileButton.style.display = 'inherit';
